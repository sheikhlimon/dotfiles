#!/bin/bash
# Portable screenshot script - works with any Hyprland setup
# (Based on omarchy-capture-screenshot, omarchy-specific bits removed)
...
# If slurp is already running, this second invocation cancels it
pkill slurp && exit 0
    ...
    if ((${BASH_REMATCH[3]} * ${BASH_REMATCH[4]} < 20)); then
      ...
      while IFS= read -r rect; do
        if [[ $rect =~ ^([0-9]+),([0-9]+)[[:space:]]([0-9]+)x([0-9]+) ]]; then
          ...
          if ((click_x >= rect_x && click_x < rect_x + rect_width && click_y >= rect_y && click_y < rect_height + rect_y)); then
...
case "$PROCESSING" in
  slurp)
    ...
    (
      ACTION=$(notify-send "Screenshot saved" "Edit with your screenshot tool" -t 10000 -i "$FILEPATH" -A "default=edit")
